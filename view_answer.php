...
<head>
	...
	$hist = $conn->query("SELECT * FROM history where quiz_id =".$_GET['id']. " and id = ".$_GET['uid'])->fetch_array();
	?>
				...
				<ul class="q-items list-group mt-4 mb-4 ?>">
					<li class="q-field list-group-item">
						...
						<?php while($orow = $opt->fetch_assoc()){ ?>
							...
							<li class="answer list-group-item <?php echo $answer['option_id'] == $orow['id'] && $answer['is_right'] == 1 ? "bg-success" : $orow['is_right'] == 1 ? "bg-success" : "white" ?>">
								<label><input type="radio" > <?php echo $orow['option_txt'] ?></label>
							</li>
						<?php } ?>
